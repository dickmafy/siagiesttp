...
CREATE OR REPLACE FUNCTION administracion.lst_solicitud()
RETURNS TABLE(pk_solicitud integer, fecha date, nombre character varying, nombres text, tipo smallint, descripcion character varying, resolucion text, fechaini date, fechafin date, estado smallint) 
AS $$
   BEGIN
		RETURN QUERY SELECT lst.pk_solicitud, lst.fecha, inst.nombre, per.apellido_paterno ||''|| per.apellido_materno || ', '|| per.nombres , lst.tipo, lst.descripcion, lst.resolucion, lst.fecha_inicio, lst.fecha_termino, lst.estado
	 	FROM administracion.m_solicitud lst
	 	INNER JOIN administracion.m_institucion inst ON lst.pk_institucion=inst.pk_institucion
	 	INNER JOIN seguridad.usuario usu ON lst.registrante=usu.pk_usuario
	 	INNER JOIN administracion.m_personal per ON per.pk_personal=usu.pertenencia
	 	WHERE lst.estado = 2 OR lst.estado = 3 OR lst.estado = 4
	 	ORDER BY lst.fecha;
	END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION administracion.lst_solicitudinstitucion(IN institucion integer)
RETURNS TABLE(pk_solicitud integer, fecha date, nombres character varying, tipo smallint, estado smallint)
AS $$
  BEGIN
	RETURN QUERY SELECT lst.pk_solicitud, lst.fecha, usu.nombres, lst.tipo, lst.estado
	FROM administracion.m_solicitud lst
	inner join administracion.m_institucion inst on lst.pk_institucion=inst.pk_institucion
	inner join seguridad.usuario usu on inst.pk_usuario=usu.pk_usuario
	where lst.pk_institucion=$1
	ORDER BY lst.fecha;
  END;
$$ LANGUAGE plpgsql;
