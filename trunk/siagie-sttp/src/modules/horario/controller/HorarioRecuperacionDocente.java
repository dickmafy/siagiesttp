...
public class HorarioRecuperacionDocente extends GenericController   
{
	...
	public void init() throws Exception
	{
		...
		page_new="hor_asis_doc";
		page_main="hor_asis_doc";
		forward(page_main);
	}
	...
	public void beforeSave() throws Exception {
		HorarioRecuperacion bean = (HorarioRecuperacion)getBean();
		bean.setEstado(1L);
		setBean(bean);	
	...
	public void afterSave() throws Exception {
		...
		asis.setSeccion(bean.getSeccion());
		asis.setAmbiente(ambienteInicial);
		asis.setFecha(bean.getSesion());
		asis.setEstado(3L);
		
		//myService.actualizarEstadoSesion(bean.getSeccion(),bean.getSesion(),ambienteInicial);
		myService.actualizarSesion(asis);
		
		bean = null;
	}
	
	public void optionGoRecuperacion() throws Exception
	{
		HorarioRecuperacion bean = new HorarioRecuperacion();
		Seccion seccion= (Seccion)getBeanSelected();
		bean.setSeccion(seccion.getId());
		bean.setAmbiente(seccion.getAmbiente());
		bean.setSesion(seccion.getFecha());
		setBean(bean);
		
		Ambiente amb = (Ambiente) getService().findById(Ambiente.class, seccion.getAmbiente());  		
		ambienteList=getListSelectItem(myService.listarAmbientes(institucion, amb.getTipo(), amb.getCapacidad()), "id", "codigo,nombre"," - ",false);
		
		ambienteInicial = seccion.getAmbiente();
		
		bean=null;
		seccion=null;
		amb=null;
		forward("docente_recuperacion");
	}
